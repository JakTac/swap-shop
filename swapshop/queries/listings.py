...
class ListingQueries:
    ...
    def create(
        self, listing: ListingIn, user_id: int
        ...
        try:
            with pool.connection() as conn:
                with conn.cursor() as db:
                    ...
                    return [
                        self.record_to_listing_out(record) for record in result
                    ]
        except Exception as e:
            print(e)
    ...
    def update(
        self, listing_id: int, listing: UpdateListing, user_id: int
    ) -> Union[ListingOut, Error]:
        with pool.connection() as conn:
            with conn.cursor() as db:
                db.execute(
                ...
                return self.update_listing_in_to_out(
                    listing=listing, listing_id=listing_id, user_id=user_id
                )

    def get_by_category(
        self, category_id: int
    ) -> Union[Error, List[ListingOut]]:
        try:
            with pool.connection() as conn:
                with conn.cursor() as db:
                    result = db.execute(
    ...
    def listing_in_to_out(
        self, listing_id: int, listing: ListingIn, sold: bool, user_id: int
    ):
        old_data = listing.dict()
        return ListingOut(
            listings_id=listing_id,
    ...
    def record_to_category_out(self, record):
        return CategoryOut(id=record[0], category=record[1])
